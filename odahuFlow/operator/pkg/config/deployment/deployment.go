...
func init() {
	...
	viper.SetDefault(DefaultRoleName, "default-odahuflow")
	config.PanicIfError(viper.BindEnv(DefaultRoleName))

	viper.SetDefault(Namespace, "odahuflow-deployment")
	...
	viper.SetDefault(ServerTemplateFolder, "odahuflow/operator/templates")
