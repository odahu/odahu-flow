...
type ModelRouteSuite struct {
	...
	g            *GomegaWithT
	server       *gin.Engine
	mdKubeClient dep_repository.Repository
}
...
func (s *ModelRouteSuite) SetupSuite() {
	...
	err := s.mdKubeClient.CreateModelDeployment(&deployment.ModelDeployment{
		ID: mdID1,
...
func (s *ModelRouteSuite) TearDownSuite() {
	for _, mdID := range []string{mdID1, mdID2} {
		if err := s.mdKubeClient.DeleteModelDeployment(mdID); err != nil && !errors.IsNotFoundError(err) {
			panic(err)
		}
	}
}
...
func (s *ModelRouteSuite) registerHTTPHandlers(deploymentConfig config.ModelDeploymentConfig) {
	...
	dep_route.ConfigureRoutes(v1Group, s.mdKubeClient, s.mdKubeClient, deploymentConfig, config.NvidiaResourceName)
}
