...
func (_m *RoutesEventGetter) Get(ctx context.Context, cursor int) ([]outbox.RouteEvent, int, error) {
	ret := _m.Called(ctx, cursor)

	var r0 []outbox.RouteEvent
	if rf, ok := ret.Get(0).(func(context.Context, int) []outbox.RouteEvent); ok {
		r0 = rf(ctx, cursor)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]outbox.RouteEvent)
		}
	}
