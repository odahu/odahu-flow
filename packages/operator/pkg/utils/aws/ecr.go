...
var log = logf.Log.WithName("odahu-flow-ecr")

// Retrieves the docker credentials from ecr registry
func ExtractEcrCreds(connSpec odahuflowv1alpha1.ConnectionSpec) (user string, password string, err error) {
...
func newSession(connSpec odahuflowv1alpha1.ConnectionSpec) (*session.Session, error) {
...
func newClientFromRegion(connSpec odahuflowv1alpha1.ConnectionSpec) (api.Client, error) {
...
func newClientWithFipsEndpoint(connSpec odahuflowv1alpha1.ConnectionSpec) (api.Client, error) {
	awsSession, err := newSession(connSpec)
	if err != nil {
		log.Error(err, "Session creation")
