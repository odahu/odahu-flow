...
type ModelTrainingRouteSuite struct {
	...
	trainStorage     mt_repository.Repository
...
func (s *ModelTrainingRouteSuite) SetupSuite() {
	...
	s.trainStorage = mt_postgres_repository.TrainingPostgresRepo{DB: db}
	s.toolchainStorage = mt_postgres_repository.ToolchainRepository{DB: db}
	s.trainService = mt_k8s_repository.NewRepository(testNamespace, testNamespace, s.k8sClient, cfg)
	...
	if err := s.toolchainStorage.CreateToolchainIntegration(&training.ToolchainIntegration{
...
func (s *ModelTrainingRouteSuite) TearDownSuite() {
	...
	if err := s.toolchainStorage.DeleteToolchainIntegration(testToolchainIntegrationID); err != nil {
...
func (s *ModelTrainingRouteSuite) TearDownTest() {
	for _, mpID := range []string{testMtID, testMtID1, testMtID2} {
		if err := s.trainStorage.DeleteModelTraining(mpID); err != nil && !odahu_errs.IsNotFoundError(err) {
...
func (s *ModelTrainingRouteSuite) registerHandlers(trainingConfig config.ModelTrainingConfig) {
	...
	train_route.ConfigureRoutes(trainGroup, trainingConfig, config.NvidiaResourceName, train_route.Services{
		Storage:          s.trainStorage,
		ConnStorage:      s.connStorage,
		ToolchainStorage: s.toolchainStorage,
		TrainService:     s.trainService,
	})
}
...
func (s *ModelTrainingRouteSuite) newMultipleMtStubs() {
	...
	s.g.Expect(s.trainStorage.CreateModelTraining(mt1)).NotTo(HaveOccurred())

	mt2 := newMtStub()
	mt2.ID = testMtID2
	mt2.Spec.Model.Version = testModelVersion2
	s.g.Expect(s.trainStorage.CreateModelTraining(mt2)).NotTo(HaveOccurred())
}
...
func (s *ModelTrainingRouteSuite) TestGetMT() {
	mt := newMtStub()
	s.g.Expect(s.trainStorage.CreateModelTraining(mt)).NotTo(HaveOccurred())
...
func (s *ModelTrainingRouteSuite) TestCreateMT() {
	...
	mt, err := s.trainStorage.GetModelTraining(testMtID)
	s.g.Expect(err).ShouldNot(HaveOccurred())
...
func (s *ModelTrainingRouteSuite) TestDeleteMT() {
	...
	mtList, err := s.trainStorage.GetModelTrainingList()
	s.g.Expect(err).NotTo(HaveOccurred())
...
func (s *ModelTrainingRouteSuite) TestSaveMTResult() {
	...
	result, err = s.trainService.GetModelTrainingResult(testMtID)
	s.g.Expect(err).NotTo(HaveOccurred())
