...
import (
	...
	pack_route "github.com/odahu/odahu-flow/packages/operator/pkg/appserver/routes/v1/packaging"
...
func (s *ModelPackagingRouteSuite) SetupSuite() {
	...
	s.packService = mp_k8s_repository.NewRepository(testNamespace, testNamespace, s.k8sClient, cfg)
	s.piStorage = mp_postgres_repository.PackagingIntegrationRepository{DB: db}
	s.packStorage = mp_postgres_repository.PackagingPostgresRepo{DB: db}

	err := s.piStorage.CreatePackagingIntegration(&packaging.PackagingIntegration{
		ID: piIDMpRoute,
...
func (s *ModelPackagingRouteSuite) TearDownSuite() {
	if err := s.piStorage.DeletePackagingIntegration(piIDMpValid); err != nil {
		s.T().Fatal(err)
	}
...
func (s *ModelPackagingRouteSuite) registerHandlers(packagingConfig config.ModelPackagingConfig) {
	...
	pack_route.ConfigureRoutes(
		packGroup, s.packService, s.packStorage,
		s.piStorage, s.connStorage, packagingConfig,
...
func (s *ModelPackagingRouteSuite) TearDownTest() {
	for _, mpID := range []string{mpIDRoute, testMpID1, testMpID2} {
		if err := s.packStorage.DeleteModelPackaging(mpID); err != nil && !errors.IsNotFoundError(err) {
...
func (s *ModelPackagingRouteSuite) createModelPackagings() {
	...
	mp1.ID = testMpID1
	s.g.Expect(s.packStorage.CreateModelPackaging(mp1)).NotTo(HaveOccurred())

	mp2 := newModelPackaging()
	mp2.ID = testMpID2
	s.g.Expect(s.packStorage.CreateModelPackaging(mp2)).NotTo(HaveOccurred())
}
...
func (s *ModelPackagingRouteSuite) TestGetMP() {
	...
	s.g.Expect(s.packStorage.CreateModelPackaging(mp)).NotTo(HaveOccurred())
...
func (s *ModelPackagingRouteSuite) TestCreateMP() {
	...
	mp, err := s.packStorage.GetModelPackaging(mpIDRoute)
...
func (s *ModelPackagingRouteSuite) TestDeleteMP() {
	...
	mpList, err := s.packStorage.GetModelPackagingList()
	s.g.Expect(err).NotTo(HaveOccurred())
...
func (s *ModelPackagingRouteSuite) TestSavingMPResult() {
	...
	result, err = s.packService.GetModelPackagingResult(mpIDRoute)
	s.g.Expect(err).NotTo(HaveOccurred())
