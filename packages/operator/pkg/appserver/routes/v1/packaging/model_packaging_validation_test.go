...
type ModelPackagingValidationSuite struct {
	suite.Suite
	g           *GomegaWithT
	mpService   mp_repository.Service
	mpStorage   mp_repository.Repository
	mpiStorage  mp_repository.PackagingIntegrationRepository
...
func (s *ModelPackagingValidationSuite) SetupSuite() {
	...
	s.mpService = mp_k8s_repository.NewRepository(testNamespace, testNamespace, kubeClient, cfg)

	s.mpStorage = mp_post_repository.PackagingPostgresRepo{DB: db}
	s.mpiStorage = mp_post_repository.PackagingIntegrationRepository{DB: db}

	s.connStorage = conn_k8s_repository.NewRepository(testNamespace, kubeClient)
	...
	err := s.mpiStorage.CreatePackagingIntegration(&packaging.PackagingIntegration{
		ID: piIDMpValid,
...
func (s *ModelPackagingValidationSuite) TearDownSuite() {
	...
	if err := s.mpiStorage.DeletePackagingIntegration(piIDMpValid); err != nil {
		s.T().Fatal(err)
	}
}
...
func (s *ModelPackagingValidationSuite) TestOutputConnection() {
	...
	// If connection packService doesn't contain connection with passed ID validation must raise NotFoundError
	mp = &packaging.ModelPackaging{
