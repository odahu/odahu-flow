...
import (
	...
	dep_kube_repository "github.com/odahu/odahu-flow/packages/operator/pkg/repository/deployment/kubernetes"
	"github.com/odahu/odahu-flow/packages/operator/pkg/appserver/routes"
	dep_route "github.com/odahu/odahu-flow/packages/operator/pkg/appserver/routes/v1/deployment"
...
type ModelRouteSuite struct {
	...
	g         *GomegaWithT
	server    *gin.Engine
	mdService dep_repository.Repository
}

func (s *ModelRouteSuite) SetupSuite() {

	s.mdService = dep_kube_repository.NewRepositoryWithOptions(
		testNamespace, kubeClient, metav1.DeletePropagationBackground,
	)

	err := s.mdService.CreateModelDeployment(&deployment.ModelDeployment{
		ID: mdID1,
...
func (s *ModelRouteSuite) TearDownSuite() {
	for _, mdID := range []string{mdID1, mdID2} {
		if err := s.mdService.DeleteModelDeployment(mdID); err != nil && !errors.IsNotFoundError(err) {
			panic(err)
		}
	}
}
...
func (s *ModelRouteSuite) registerHTTPHandlers(deploymentConfig config.ModelDeploymentConfig) {
	...
	dep_route.ConfigureRoutes(v1Group, s.mdService, s.mdService, deploymentConfig, config.NvidiaResourceName)
}

func (s *ModelRouteSuite) TearDownTest() {
	for _, mdID := range []string{mrID, mrID1, mrID2} {
		if err := s.mdService.DeleteModelRoute(mdID); err != nil && !errors.IsNotFoundError(err) {
			panic(err)
		}
	}
}
...
func (s *ModelRouteSuite) TestGetMR() {
	mr := newStubMr()
	s.g.Expect(s.mdService.CreateModelRoute(mr)).NotTo(HaveOccurred())
...
func (s *ModelRouteSuite) TestGetAllModelRoutes() {
	conn := newStubMr()
	s.g.Expect(s.mdService.CreateModelRoute(conn)).NotTo(HaveOccurred())
...
func (s *ModelRouteSuite) TestGetAllModelRoutesPaging() {
	mr1 := newStubMr()
	mr1.ID = mrID1
	s.g.Expect(s.mdService.CreateModelRoute(mr1)).NotTo(HaveOccurred())

	mr2 := newStubMr()
	mr2.ID = mrID2
	s.g.Expect(s.mdService.CreateModelRoute(mr2)).NotTo(HaveOccurred())
...
func (s *ModelRouteSuite) TestCreateMR() {
	...
	mr, err := s.mdService.GetModelRoute(mrID)
...
func (s *ModelRouteSuite) TestDeleteMR() {
	...
	mrList, err := s.mdService.GetModelRouteList()
	s.g.Expect(err).NotTo(HaveOccurred())
