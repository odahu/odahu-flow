...
def test_list_local_trainings(tmpdir):
    ...
    with patch("odahuflow.sdk.local.training.config.LOCAL_MODEL_OUTPUT_DIR", tmpdir):
        assert training_sdk.list_local_trainings() == [
            "&wine-1-12",
            "2Wine-1@-12",
            "@wine-1@-12",
            "Awine-1@",
...
def test_local_training_relative_output_dir(mocker: MockFixture, cli_runner: CliRunner):
    ...
    trainer_mock = mocker.patch.object(
        training, "K8sTrainer", autospec=True
    ).return_value
    ...
    docker_mock: Mock = mocker.patch.object(
        training_sdk.docker, "from_env"
    ).return_value
    ...
    with tempfile.TemporaryDirectory(dir=os.curdir) as temp_dir:
        ...
        training_yml.write_text(
            json.dumps(
                {
                    **ModelTraining(
                        id="training1", spec=ModelTrainingSpec(toolchain="toolchain1")
                    ).to_dict(),
                    **{"kind": "ModelTraining"},
                }
            )
        ...
        toolchain_yml.write_text(
            json.dumps(
                {
                    **ToolchainIntegration(id="toolchain1").to_dict(),
        ...
        assert (
            result.exit_code == 0
        ), f"command invocation ended with exit code {result.exit_code}"
        assert (
            result.exception is None
        ), f"command invocation ended with exception {result.exception}"
        ...
        for call in docker_mock.containers.run.call_args_list:
            ...
            for mount in filter(
                lambda m: m.get("Target") == MODEL_OUTPUT_CONTAINER_PATH, mounts
            ):
