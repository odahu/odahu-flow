...
from odahuflow.cli.utils.output import JSON_OUTPUT_FORMAT, JSONPATH_OUTPUT_FORMAT
...
def test_get_jsonpath(
    ...
    result = cli_runner.invoke(
        entity_test_data.click_group,
        ["get", "--id", ENTITY_ID, "-o", f"{JSONPATH_OUTPUT_FORMAT}=[*].id"],
        obj=entity_test_data.entity_client,
    )
    client_mock.assert_called_once_with(ENTITY_ID)
    assert result.exit_code == 0
...
def test_get_wrong_output_format(
    cli_runner: CliRunner, entity_test_data: EntityTestData
):
    result = cli_runner.invoke(
        entity_test_data.click_group,
...
def test_edit_wrong_kind(
    tmp_path: pathlib.Path, cli_runner: CliRunner, entity_test_data: EntityTestData
):
...
def test_delete_id_and_file_present(
    tmp_path, cli_runner: CliRunner, entity_test_data: EntityTestData
):
    entity_file = tmp_path / "entity.yaml"
    entity_file.write_text(
        json.dumps(
...
def test_delete_no_id_or_file_present(
    tmp_path, cli_runner: CliRunner, entity_test_data: EntityTestData
):
    entity_file = tmp_path / "entity.yaml"
    entity_file.write_text(
    ...
    result = cli_runner.invoke(
        entity_test_data.click_group, ["delete"], obj=entity_test_data.entity_client
    )
...
def test_delete_ignore_not_found_enabled(
    mocker: MockFixture, cli_runner: CliRunner, entity_test_data: EntityTestData
):
    client_mock = mocker.patch.object(
        entity_test_data.entity_client.__class__,
    ...
    result = cli_runner.invoke(
        entity_test_data.click_group,
        ["delete", "--id", ENTITY_ID, "--ignore-not-found"],
        obj=entity_test_data.entity_client,
    )
    ...
    assert result.exit_code == 0
    assert (
        IGNORE_NOT_FOUND_ERROR_MESSAGE.format(kind=entity_test_data.kind, id=ENTITY_ID)
        in result.stdout
    )


def test_delete_ignore_not_found_disabled(
    mocker: MockFixture, cli_runner: CliRunner, entity_test_data: EntityTestData
):
    client_mock = mocker.patch.object(
        entity_test_data.entity_client.__class__,
