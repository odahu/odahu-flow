...
from odahuflow.sdk.local.packaging import (
    start_package,
...
def fetch_local_entities(
    manifest_file: List[str], manifest_dir: List[str]
) -> List[OdahuflowCloudResourceUpdatePair]:
    """
...
def _decode_connection(connection: Connection) -> None:
    ...
    for f in decode_fields:
        ...
        try:
            ...
            LOGGER.error(
                f"Unable to decode base64 .spec.{f} of connection {connection.id}"
            )
            raise e
        ...
        try:
            ...
            LOGGER.error(
                f"Unable to decode utf-8 .spec.{f} of connection {connection.id}"
            )
            raise e
...
def get_packager(
    name: str,
    local: Dict[str, PackagingIntegration],
    remote_api: PackagingIntegrationClient,
) -> PackagingIntegration:
    """
...
def get_packager_targets(
    ...
    for t in targets:
        ...
        packager_targets.append(PackagerTarget(conn, t.name))
...
def _deprecation_warning(is_target_disabled: bool):
    if is_target_disabled is None:
        click.echo(
            "[FeatureWarning] Current behavior is to disable all packager targets for a local run. "
        ...
        click.echo(
            "[FeatureWarning] --disable-package-targets/--no-disable-package-targets options "
...
def validate_targets(
    ...
    for t in targets:
        ...
        if t_schema is None:
            errors.append(
                f'cannot find "{t.name}" target in "{pi.id}" packaging integration'
            )
            continue
        ...
        try:
            ...
            if e.status_code == 404:
                errors.append(
                    f'"{t.connection_name}" connection of "{t.name}" target is not found'
                )
                continue
            raise e
        ...
        if pt.connection.spec.type not in t_schema.connection_types:
            errors.append(
                f'"{pt.name}" target has invalid connection type, "{pt.connection.spec.type}", '
    ...
    if errors:
        raise ClickException(
            "Next errors were found during .spec.targets validation:\n"
            + "\n".join(errors)
        )
...
@click.option("--artifact-path", type=click.Path(), help="Path to a training artifact")
@click.option(
    "--artifact-name", "-a", type=str, help="Override artifact name from file"
)
# TODO Breaking Changes: remove --disable-package-targets/--no-disable-package-targets' options
@click.option(
    "--disable-package-targets/--no-disable-package-targets",
    "is_target_disabled",
    default=None,
    help="Disable all targets in packaging",
)
@click.option("--disable-target", multiple=True, help="Disable target in packaging")
@pass_obj
def run(
    ...
    artifact_path: str,
    artifact_name: str,
    is_target_disabled: bool,
    disable_target: List[str],
):
    """
    \b
    ...
    entities: List[OdahuflowCloudResourceUpdatePair] = fetch_local_entities(
    ...
    packager = get_packager(
        mp.spec.integration_name,
        packagers,
        PackagingIntegrationClient.construct_from_other(client),
    )
    ...
    if is_target_disabled:
        ...
        targets = validate_targets(
            ...
            disable_target,
            connections,
            ConnectionClient.construct_from_other(client),
        )
