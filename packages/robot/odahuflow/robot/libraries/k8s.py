...
class K8s:
    ...
    def __init__(self, namespace):
        ...
        self._model_training_info = (
            self._odahuflow_group,
            self._model_training_version,
            self._namespace,
            self._model_training_plural,
        )
        self._model_deployment_info = (
            self._odahuflow_group,
            "v1alpha1",
            self._namespace,
            "modeldeployments",
    ...
    def get_model_training_logs(self, name):
        ...
        return core_api.read_namespaced_pod_log(
            f"{name}-training-pod", self._namespace, container="builder"
        )
    ...
    def check_all_containers_terminated(self, name):
        ...
        for container in pod.status.container_statuses:
            if not container.state.terminated:
                raise Exception(
                    f"Container {container.name} of {pod.metadata.name} pod is still alive"
                )
    ...
    def get_model_training_status(self, name):
        ...
        model_training = crds.get_namespaced_custom_object(
            *self._model_training_info, name.lower()
        )
    ...
    def get_model_deployment_status(self, name):
        ...
        md = crds.get_namespaced_custom_object(
            *self._model_deployment_info, name.lower()
        )
    ...
    def get_number_of_md_replicas(self, name, expected_number_of_replicas):
        ...
        return (
            self.get_deployment_replicas(md_status["deployment"], self._namespace)
    ...
    def wait_model_deployment_replicas(self, name, expected_number_of_replicas):
        ...
        """
        if not wait_until(
            lambda: self.get_number_of_md_replicas(name, expected_number_of_replicas),
    ...
    def deployment_is_running(self, deployment_name, namespace):
        ...
        if deployment.status.replicas != deployment.status.ready_replicas:
            raise Exception(
                "Deployment '%s' is not ready: %d/%d replicas are running"
                % (
                    deployment_name,
    ...
    def get_model_deployment(self, deployment_name, namespace):
        ...
        return extension_api.read_namespaced_deployment(
            deployment_name, namespace or self._namespace
        )
    ...
    def get_deployment_replicas(self, deployment_name, namespace="default"):
        ...
        print(
            "Scale data for {} in {} ns is {}".format(
                deployment_name, namespace, scale_data
            )
        )
        if scale_data is not None:
            return (
                0
                if not scale_data.status.available_replicas
                else scale_data.status.available_replicas
            )
        else:
            return None

    def wait_nodes_scale_down(
        self, node_taint_key, node_taint_value, timeout=600, sleep=60
    ):
        """
        ...
        while True:
            ...
            if nodes_num == 0:
                print(
                    "Scaled node was successfully unscaled after {} seconds".format(
                        elapsed
                    )
                )
                return
                ...
                raise Exception(
                    "Node was not unscaled after {} seconds wait".format(timeout)
                )
            else:
                print(
                    f"Current node count {nodes_num}. Sleep {sleep} seconds and try again"
                )
                time.sleep(sleep)
