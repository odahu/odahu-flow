...
class OdahuKubeReporter:
    ...
    def report_training_pods(self, name: str) -> str:
        ...
        try:
            model_training = crds.get_namespaced_custom_object(group, version, training_namespace,
                                                               training_plural, name.lower())
    ...
    def report_packaging_pods(self, name: str) -> str:
        ...
        try:
            model_packaging = crds.get_namespaced_custom_object(group, version, packaging_namespace,
                                                               packaging_plural, name.lower())
    ...
    def report_model_deployment_pods(self, name: str) -> str:
        ...
        try:
            model_deployment = crds.get_namespaced_custom_object(group, version, deployment_namespace,
                                                               deployment_plural, name.lower())
            report += f'Model deployment: {model_deployment}\n'
        except kubernetes.client.rest.ApiException as e:
            report += f'Model deployment: unable to retrieve. {_api_exc_print(e)}\n'
            return report

        pods = core_api.list_namespaced_pod(deployment_namespace, label_selector=f'{ODAHU_DEPL_LABEL}={name}')
