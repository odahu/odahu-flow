...
Verifying misconfused cli parameters is not permitted
    ...
    ${res}=  Shell  odahuflowctl --verbose login --url ${API_URL} --token "not-valid-token" --client_id "client-id"
    Should not be equal  ${res.rc}  ${0}
    should contain  ${res.stderr}  You should use either --token or --client_id/--client_secret to login
...
User try to make client_credentials login without passing issuer param
    [Documentation]
    ${res}=  Shell  odahuflowctl --verbose login --url ${API_URL} --client_id ${SA_CLIENT_ID} --client_secret ${SA_CLIENT_SECRET}
    Should not be equal  ${res.rc}  ${0}
...
User try to make client_credentials login without passing client_secret param
    [Documentation]
    ${res}=  Shell  odahuflowctl --verbose login --url ${API_URL} --client_id ${SA_CLIENT_ID} --issuer ${ISSUER}
    Should not be equal  ${res.rc}  ${0}
