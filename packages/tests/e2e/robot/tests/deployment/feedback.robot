...
Force Tags          deployment  cli  feedback
...
*** Keywords ***
Cleanup resources
    StrictShell  odahuflowctl --verbose dep delete --id ${MD_FEEDBACK_MODEL} --ignore-not-found
...
Send feedback for deployed model
    ...
    ${resp}=              Invoke model feedback  ${md_name}  ${model_name}  ${model_ver}  ${API_URL}  ${AUTH_TOKEN}  ${request_id}  &{arguments}
    [Return]              ${resp}
