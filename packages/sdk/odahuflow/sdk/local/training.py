...
def compile_artifact_name_template(go_template: str, context: TemplateContext) -> str:
    ...
    py_template = re.sub(
        r"{{\s*?\.([a-zA-Z_0-9]+)\s*?}}", lambda x: f"{{ctx.{x.group(1)}}}", go_template
    )
...
def launch_training_container(trainer: K8sTrainer, output_dir: str) -> None:
    ...
    container: Container = client.containers.run(
        trainer.model_training.spec.image
        or trainer.toolchain_integration.spec.default_image,
        stderr=True,
        stdout=True,
...
def create_mt_config_file(trainer: K8sTrainer) -> None:
    with open(TRAINER_CONF_PATH, "w") as f:
        ...
        LOGGER.debug(
            f"Saved the trainer configuration:\n{json.dumps(trainer_dict, indent=2)}"
        )
...
def start_train(trainer: K8sTrainer, output_dir: str) -> None:
    ...
    if not output_dir:
        ...
        LOGGER.debug(
            f"Output directory for model training is not provided. Using default: {output_dir}"
        )

    model_dir_name_template = (
        trainer.model_training.spec.model.artifact_name_template
        or DEFAULT_MODEL_DIR_TEMPLATE
    )
    ...
    model_dir_name = compile_artifact_name_template(
        ...
        context=TemplateContext(
            Name=trainer.model_training.spec.model.name,
