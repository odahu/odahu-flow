...
from odahuflow.sdk.local.training import (
    compile_artifact_name_template,
    TemplateContext,
    start_train,
...
@pytest.mark.parametrize(
    ...
    [(None, DEFAULT_OUTPUT_DIR), ("/custom/output", "/custom/output")],
)
def test_start_training__output_dir(
    output_dir: Optional[str], expected_output_dir: str, mocker: MockFixture
):
    """
    ...
    """
    trainer = K8sTrainer(
        model_training=ModelTraining(spec=ModelTrainingSpec(model=ModelIdentity()))
    )
    ...
    compile_artifact_name_template_mock = mocker.patch.object(
        training, "compile_artifact_name_template"
    )
    launch_training_container_mock = mocker.patch.object(
        training, "launch_training_container"
    )
    launch_gppi_validation_container_mock = mocker.patch.object(
        training, "launch_gppi_validation_container"
    )
    ...
    launch_gppi_validation_container_mock.assert_called_with(
        trainer, expected_model_dir_path
    )
...
def test_start_training__artifact_name_template(
    template: Optional[str], expected_template: str, mocker: MockFixture
):
    """
    ...
    trainer = K8sTrainer(
        model_training=ModelTraining(spec=ModelTrainingSpec(model=model))
    )
    ...
    compile_artifact_name_template_mock.assert_called_with(
        go_template=expected_template, context=ANY
    )
