...
test_data = [
    ("wine-1.0", "/odahu/training", "wine-1.0", "/odahu/training"),
    ("wine-1.0.zip", "/odahu/training", "wine-1.0", "/odahu/training"),
    ("wine-1.0.zip.zip", None, "wine-1.0.zip", "/odahu/default_output"),
]
...
def test_start_package__artifact_name_artifact_path(
    artifact_name,
    artifact_path,
    expected_artifact_name,
    expected_artifact_path,
    ...
    packager = K8sPackager(
        model_packaging=ModelPackaging(
            spec=ModelPackagingSpec(artifact_name=artifact_name)
        ),
        # mocking packaging_integration default_image
        packaging_integration=PackagingIntegration(
            spec=PackagingIntegrationSpec(default_image="default_image")
        ),
    )
    ...
    expected_packager = K8sPackager(
        model_packaging=ModelPackaging(
            spec=ModelPackagingSpec(artifact_name=expected_artifact_name)
        ),
        # mocking packaging_integration default_image
        packaging_integration=PackagingIntegration(
            spec=PackagingIntegrationSpec(default_image="default_image")
    ...
    expected_full_artifact_path = os.path.join(
        expected_artifact_path, expected_artifact_name
    )

    create_mp_config_file_mock.assert_called_with(
        expected_full_artifact_path, expected_packager
    )
